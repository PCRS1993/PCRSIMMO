		...
		class constructera_Welcome {
			...
			public function __construct( $plugins, $strings ) {
				...
				$this->tab_sections = array(
					...
					'recommended_plugins' => esc_html__('Recommended Plugins', 'zigcy-lite'),
			...
			public function activation_admin_notice() {
				...
				if( is_admin() && ('themes.php' == $pagenow) && (isset($_GET['activated'])) ) { ?>
					<div class="notice notice-success is-dismissible">
						<p>
							<?php
								printf( '%1$s %2$s %3$s <a href="%4$s">%5$s</a> %6$s', esc_html__( 'Welcome! Thank you for choosing', 'zigcy-lite' ), esc_html($this->theme_name), esc_html__( 'Please make sure you visit our', 'zigcy-lite' ), esc_url( admin_url( 'themes.php?page=zigcy-lite-welcome' ) ), esc_html__( 'Welcome Page', 'zigcy-lite' ), esc_html__( 'to get started with Swing.', 'zigcy-lite' ) );
							?>
						</p>
						<p><a class="button" href="<?php echo esc_url(admin_url( 'themes.php?page=zigcy-lite-welcome' )) ?>"><?php esc_html_e( 'Lets Get Started', 'zigcy-lite' ); ?></a></p>
					</div>
					<?php
				}
			}
			...
			public function welcome_register_menu() {
				...
				add_theme_page( esc_html($this->strings['welcome_menu_text']), $title , 'edit_theme_options', 'zigcy-lite-welcome', array( $this, 'welcome_screen' ));
			}
			...
			public function welcome_screen() {
				...
				<div class="wrap about-wrap access-wrap">
					<h1><?php /* translators: %1$s: theme name, %2$s: theme version */ printf( esc_html__( 'Welcome to %1$s - Version %2$s', 'zigcy-lite' ), esc_html($this->theme_name), esc_html($this->theme_version) ); ?></h1>
					<div class="about-text"><?php echo esc_html($this->strings['theme_short_description']); ?></div>

					<a target="_blank" href="http://www.accesspressthemes.com" class="accesspress-badge wp-badge"><span><?php echo esc_html('AccessPressThemes', 'zigcy-lite'); ?></span></a>
				...
				<div class="nav-tab-wrapper clearfix">
					<?php foreach($tabs as $id => $label) : ?>
						...
						?>
						<a href="<?php echo esc_url(admin_url('themes.php?page=zigcy-lite-welcome&section='.$id)); ?>" class="<?php echo esc_attr($nav_class); ?>" >
			...
			public function welcome_styles_and_scripts() {
				wp_enqueue_style( 'zigcy-lite' . '-welcome-screen', get_template_directory_uri() . '/inc/welcome/css/welcome.css' );
				wp_enqueue_script( 'zigcy-lite' . '-welcome-screen', get_template_directory_uri() . '/inc/welcome/js/welcome.js', array( 'jquery' ) );

				wp_localize_script( 'zigcy-lite' . '-welcome-screen', 'ZigcyLiteWelcomeObject', array(
			...
			public function get_plugin_active($plugin) {
				...
				$zigcy_path = WP_PLUGIN_DIR.'/'.esc_attr($folder_name).'/'.esc_attr($file_name);
				if( file_exists( $zigcy_path ) ) {
			...
			public function all_required_plugins_installed() {
		  	...
		  	public function get_local_dir_path($plugin) {
	      		...
	      		$url = wp_nonce_url(admin_url('themes.php?page=' . 'zigcy-lite' . '-welcome&section=actions_required'),'remote-file-installation');
